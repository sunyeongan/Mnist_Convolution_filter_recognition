...
def resized(image):
    r1 = 44.0 / image.shape[1]
    r2 = 18.5 / image.shape[0]
    print("image.shape[1] : ", int(image.shape[1] * r2))
    print("image.shape[0] : ", int(image.shape[0] * r1))
    dim1 = (int(image.shape[1] * r2), int(image.shape[0] * r1))

    resized_original = cv2.resize(image, dim1, interpolation=cv2.INTER_AREA)
    #이미지 축소시에 많이 사용하는 보간법은 INTER_AREA 이다.
    return resized_original

def command(mode):
    #1 입력시 블러링 필터 적용
    #2 입력시 샤프닝 필터 적용
    global icons, image, canvas, Color, hue, mouse_mode,count,imageH,imageW

    if mode == PALETTE:  # 색상팔레트 영역 클 릭 시
        pixel = image[pt2[::-1]]
        x, y, w, h = icons[COLOR]
        ...
        blue, green, red = cv2.split(image) #rgb 영역 분리
        print("original img size : ",canvas.shape)
        resized_wonbon = resized(canvas)
        cv2.imwrite("OutputImage/my_save_" + str(count) + ".jpg", resized_wonbon)#원본출력
        ...
        OpencvFilter = cv2.filter2D(image, -1, mask1)
        OpencvFilter2 = cv2.filter2D(image, -1, mask2)

        blur = cv2.filter2D(resized_wonbon,-1,mask1)
        sharp = cv2.filter2D(resized_wonbon,-1,mask2)
        gaussian_blur = cv2.GaussianBlur(resized_wonbon,(1,1),0)
        canny2 = cv2.Canny(resized_wonbon,100,150)

        #canvas resize 28*28


        cv2.imwrite("OutputImage/bluring_"+str(count)+".jpg",blur)
        cv2.imwrite("OutputImage/sharp_"+str(count)+".jpg",sharp)
        cv2.imwrite("OutputImage/gaussian_blur_"+str(count)+".jpg",gaussian_blur)
        cv2.imwrite("OutputImage/canny_" + str(count) + ".jpg", canny2)
        count += 1
...
canvas = image[h * 1:image.shape[0]:, w * 2:image.shape[1]]# 메뉴를 제외한 캔버스 영역
#image[h*2:image.shape[1], w * 2:image.shape[0]]
